...
class Exercise:
    ...
    def _check_angle_shoulder_left_flexion_extension(self, angle: list, target_state: AngleTargetStates, tolerance: int = 10) -> dict:
        if target_state not in self.angles.keys():
        ...
        target_end = self.angles[AngleTargetStates.END]["shoulder_left"]["flexion_extension"]["angle"][1]
        target_start = self.angles[AngleTargetStates.START]["shoulder_left"]["flexion_extension"]["angle"][1]
        ...
        target_min = min(self.angles[target_state]["shoulder_left"]["flexion_extension"]["angle"])
        target_max = max(self.angles[target_state]["shoulder_left"]["flexion_extension"]["angle"])

        result_state = AngleAnalysisResultStates.NONE
        if target_state == AngleTargetStates.START:
            if angle < target_min - tolerance:
                result_state = AngleAnalysisResultStates.TARGET_EXCEEDED if target_end_is_flexion else result_state
                result_state = AngleAnalysisResultStates.TARGET_UNDERCUT if target_end_is_extension else result_state
                result_state = AngleAnalysisResultStates.TARGET_EXCEEDED if target_end_is_zero else result_state
            if angle > target_max + tolerance:
                result_state = AngleAnalysisResultStates.TARGET_UNDERCUT if target_end_is_flexion else result_state
                result_state = AngleAnalysisResultStates.TARGET_EXCEEDED if target_end_is_extension else result_state
                result_state = AngleAnalysisResultStates.TARGET_EXCEEDED if target_end_is_zero else result_state
            if angle <= target_max + tolerance and angle >= target_min - tolerance:
                result_state = AngleAnalysisResultStates.IN_TARGET_RANGE
